...
import com.andb.apps.todo.util.swap
...
class AddTask(ctxt: Context, val activity: MainActivity) : FrameLayout(ctxt) {
    ...
    private fun theme() {
        ...
        addTaskName.apply {
            ...
            setCursorColor(Utilities.sidedLighterDarker(Cyanea.instance.backgroundColor, .6f))
        }
        checkDividers()
    }
    ...
    private fun checkDividers() {
        addTaskDivider1.visibility = if (task.listItems.isNotEmpty()) {
            View.VISIBLE
        } else {
            View.GONE
        }
        addTaskDivider2.visibility = if (task.listTags.isNotEmpty()) {
            View.VISIBLE
        } else {
            View.GONE
        }
    ...
    private fun tagAdapter() = Klaster.get()
        ...
        .bind { _ ->
            ...
            itemView.apply {
                tagImage.setColorFilter(tag.color)
                task_view_item_tag_name.text = tag.name
                setOnClickListener {
        ...
        .bind { _ ->
            itemView.apply {
                taskItemEditText.apply {
                    ...
                    setCursorColor(Utilities.lighterDarker(Cyanea.instance.backgroundColor, .6f))
                }
        ...
        override fun onMove(recyclerView: RecyclerView, viewHolder: RecyclerView.ViewHolder, target: RecyclerView.ViewHolder): Boolean {
            ...
            if (fromPosition < toPosition) {
                for (i in fromPosition until toPosition) {
                    task.listItems.swap(i, i + 1)
                    task.listItemsChecked.swap(i, i + 1)
                }
            } else {
    ...
    fun onAddTaskAddTagEvent(event: AddTaskAddTagEvent) {
        if (!task.listTags.contains(event.tag)) {
            ...
            tagAdapter.notifyItemInserted(task.listTags.size - 1)
            checkDividers()
        } else {
